	...
	@if ($hero)
		<div class="overflow-hidden">
			<div class="mx-auto max-w-[85rem] px-4 py-20 sm:px-6 lg:px-8">
              ...
              {{ $hero['hero_heading'] }} <span class="text-blue-500"> {{ $hero['hero_heading_focus'] }}</span>
						</h1>
					</div>
					<!-- End Title -->
					...
					<div class="text-center sm:flex sm:items-center sm:justify-center sm:text-start">
						...
						<div class="mx-auto h-px w-32 border-t border-gray-200 sm:mx-0 sm:h-full sm:w-auto sm:border-s sm:border-t-0">
						</div>

						<div class="pt-5 sm:ps-5 sm:pt-0">
							<div class="text-lg font-semibold text-gray-800">{{ $hero['hero_feature_text'] }}</div>
							<div class="text-sm text-gray-500">{{ $hero['hero_feature_subtext'] }}</div>
						</div>
					</div>
					<!-- End Avatar Group -->
					...
					<!-- Form -->
					<form>
						<div class="mx-auto max-w-2xl rounded-lg border bg-white p-3 shadow-lg shadow-gray-100 sm:flex sm:space-x-3">
							<div class="pb-2 sm:flex-[1_0_0%] sm:pb-0">
								<label for="hs-hero-name-1" class="block text-sm font-medium"><span
										class="sr-only">{{ $hero['hero_form']['hero_form_name_label'] }}</span></label>
								<input type="text" id="hs-hero-name-1"
									class="block w-full rounded-lg border-transparent px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500"
									placeholder={{ $hero['hero_form']['hero_form_name_placeholder'] }}>
							</div>
							<div
								class="border-t border-gray-200 pt-2 sm:flex-[1_0_0%] sm:border-s sm:border-t-0 sm:ps-3 sm:pt-0">
								<label for="hs-hero-email-1" class="block text-sm font-medium"><span
										class="sr-only">{{ $hero['hero_form']['hero_form_email_label'] }}</span></label>
								<input type="email" id="hs-hero-email-1"
									class="block w-full rounded-lg border-transparent px-4 py-3 text-sm focus:border-blue-500 focus:ring-blue-500"
									placeholder={{ $hero['hero_form']['hero_form_email_placeholder'] }}>
							</div>
							<div class="grid pt-2 sm:block sm:flex-[0_0_auto] sm:pt-0">
								<a
									...
									href={{ $hero['hero_button']['hero_button_link'] }}>
									{{ $hero['hero_button']['hero_button_text'] }}
								</a>
							</div>
						</div>
					</form>
					...
					<!-- SVG Element -->
					<div class="absolute start-0 top-2/4 hidden -translate-x-40 -translate-y-2/4 transform md:block lg:-translate-x-80"
						aria-hidden="true">
					...
					<!-- SVG Element -->
					<div class="absolute end-0 top-2/4 hidden -translate-y-2/4 translate-x-40 transform md:block lg:translate-x-80"
						aria-hidden="true">
